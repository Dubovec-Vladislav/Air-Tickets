...
export const Ticket: FC<TicketProps> = (
  { price, origin, origin_name,
    destination, destination_name,
    departure_date, departure_time,
    arrival_date, arrival_time, stops,
    activeIndexOFCurrency }
) => {

  const currencies = ['₽', '$', '€'];
  const conversionRates = [1, 98, 105];
  const calculatedPrice = `${Math.round(price / conversionRates[activeIndexOFCurrency])} ${currencies[activeIndexOFCurrency]}`;
    ...
    <div className={style.block}>
      <div className={style.body}>
        <div className={style.left}>
          ...
          <div className={style.btn}>Купить за {calculatedPrice}</div>
        </div>
